...
STRINGS = {
    ...
    "phishing_templates": [
        ...
        {
            "id": "supply_chain",
            "name": "Compromission de fournisseur de la chaîne d'approvisionnement",
            "subject_lines": [
                "Urgent : correctif de sécurité {software} disponible - Installez avant le {deadline}",
                "[{software}] Mise à jour critique requise - Action nécessaire",
                "Obligatoire : mise à jour de sécurité éditeur {software} - Réf. : {reference}",
                "{software} Mise à jour d'intégration - Votre autorisation est requise",
            ],
            "body": "Bonjour {first_name},\n\nNous vous contactons pour vous informer d'une mise à jour de sécurité critique pour {software} nécessitant une action immédiate de la part de tous les utilisateurs autorisés.\n\nDans le cadre de notre partenariat continu avec {company}, nous avons identifié une vulnérabilité dans le déploiement actuel qui doit être corrigée avant le {deadline}.\n\nVeuillez télécharger et installer la mise à jour en utilisant le lien sécurisé ci-dessous :\n\n{phishing_link}\n\nCette mise à jour a été examinée et approuvée par votre service informatique. Si vous rencontrez des difficultés lors de l'installation, contactez votre responsable de compte {software}.\n\n{signature}",
        },
        {
            "id": "credential_breach",
            "name": "Notification de fuite d'identifiants",
            "subject_lines": [
                "ALERTE DE SÉCURITÉ : vos identifiants {company} trouvés dans une fuite de données",
                "Action requise : réinitialisation du mot de passe suite à une fuite de données tierce",
                "Urgent : votre compte {software} pourrait être compromis",
                "[Équipe Sécurité] Réinitialisation obligatoire du mot de passe - Fuite détectée",
            ],
            "body": "Cher/Chère {first_name},\n\nNotre surveillance de sécurité a détecté que des identifiants associés à votre compte {company} ont été trouvés dans une fuite de données tierce. Il ne s'agit PAS d'une compromission des systèmes de {company}, mais vos identifiants ont pu être exposés via un service externe compromis.\n\nPour protéger votre compte, vous devez réinitialiser votre mot de passe immédiatement :\n\n{phishing_link}\n\nSi vous ne réinitialisez pas votre mot de passe avant le {deadline}, votre compte sera temporairement verrouillé par mesure de précaution.\n\nPour information, les données compromises ont été identifiées le {date} et incluent des adresses e-mail et des mots de passe hachés. Nous vous recommandons fortement d'activer l'authentification multi-facteurs après la réinitialisation de votre mot de passe.\n\n{signature}",
        },
    ],
    ...
    "vishing_scripts": [
        ...
        },
        {
            ...
            "name": "Réinitialisation de mot de passe / Escalade de privilèges",
            "opening": "[APPELANT] : Bonjour, ici {caller_name} du service Gestion des identités et des accès de {company}.\nJe vous appelle car votre compte a été signalé pour une revue de privilèges obligatoire.\nSuis-je bien en ligne avec {target_name} du service {department} ?",
            "escalation": "[APPELANT] : Très bien. Voici la situation — lors de notre revue trimestrielle des accès,\nnous avons constaté que votre compte dispose de privilèges élevés qui doivent être recertifiés.\nJ'ai votre ticket sous les yeux — n°{ticket_number}, ouvert par {manager_name}.\nPour compléter la recertification, j'ai besoin que vous vérifiiez votre niveau d'accès actuel\net ensuite nous procéderons ensemble au processus de réinitialisation.\nPouvez-vous me confirmer votre rôle actuel et les groupes d'administration auxquels vous appartenez ?",
            "objective": "OBJECTIF : Amener la cible à :\n  - Révéler son niveau d'accès et ses appartenances aux groupes d'administration\n  - Naviguer vers un faux portail IAM pour la « recertification »\n  - Saisir ses identifiants actuels pour « vérification avant réinitialisation »\n  - Approuver une fausse demande MFA (attaque par notification push)\n  - Révéler les identifiants des comptes de service qu'elle gère",
            "red_flags_to_avoid": "MAINTENIR LA CRÉDIBILITÉ :\n  \u2717 Ne prétendez pas que le compte est compromis - parlez d'une revue de routine\n  \u2717 Ne demandez pas directement les mots de passe - utilisez le « portail de recertification »\n  \u2717 Ne précipitez pas le processus - les équipes IAM sont méthodiques\n  \u2717 N'appelez pas sans connaître les outils d'administration réels de la cible\n  \u2713 Connaissez les vrais noms de groupes d'administration (issus de l'OSINT, LinkedIn, offres d'emploi)\n  \u2713 Faites référence aux vrais outils IAM utilisés par l'entreprise (Azure AD, Okta, CyberArk)\n  \u2713 Ayez un identifiant d'appelant usurpé correspondant au poste de l'équipe IAM/Identité\n  \u2713 Soyez prêt à expliquer le processus de recertification en détail",
        },
        {
            "id": "service_account_audit",
            "name": "Audit de sécurité des comptes de service",
            "opening": "[APPELANT] : Bon{time_of_day}, ici {caller_name} du Centre opérationnel de sécurité de {company}.\nJe vous contacte au sujet de l'audit des comptes de service imposé par la conformité.\nJ'ai besoin de parler à la personne qui gère les comptes de service {software} dans le service {department}.",
            "escalation": "[APPELANT] : Très bien, dans le cadre de nos exigences de conformité SOC2/ISO 27001,\nnous auditons tous les comptes de service de l'organisation.\n{manager_name} du service {department} a été informé de cet audit il y a deux semaines.\nJ'ai besoin de vérifier les comptes de service liés à votre déploiement {software} —\nplus précisément les noms de comptes, le calendrier de rotation et les personnes ayant accès.\nPouvez-vous me retrouver ces informations ? J'aurai également besoin que vous vérifiiez\nla validité des identifiants actuels en les testant via notre portail d'audit.",
            "objective": "OBJECTIF : Amener la cible à :\n  - Divulguer les noms et configurations des comptes de service\n  - Révéler le calendrier de rotation des mots de passe et la date du dernier changement\n  - Tester les identifiants via un « portail d'audit » contrôlé par l'attaquant\n  - Partager les clés API ou jetons utilisés par les comptes de service\n  - Accorder un accès temporaire à la console de gestion des comptes de service",
            "red_flags_to_avoid": "MAINTENIR LA CRÉDIBILITÉ :\n  \u2717 Ne prétendez pas être d'un cabinet d'audit externe sans préparation\n  \u2717 Ne demandez pas tous les comptes de service d'un coup - commencez par un seul système\n  \u2717 N'utilisez pas de jargon de conformité que vous ne pouvez pas expliquer si on vous questionne\n  \u2717 N'omettez pas la référence à un responsable ayant approuvé l'audit\n  \u2713 Renseignez-vous sur les référentiels de conformité réellement suivis par l'entreprise\n  \u2713 Connaissez les conventions de nommage des comptes de service (messages d'erreur, offres d'emploi)\n  \u2713 Faites référence aux vraies échéances d'audit (SOC2 Type II est annuel)\n  \u2713 Proposez d'envoyer la demande d'audit par e-mail à l'aspect officiel comme vérification",
        },
    ],
    ...
    "physical_pretexts": [
        ...
        },
        {
            ...
            "name": "Technicien photocopieur / imprimante",
            "appearance": "Tenue professionnelle avec polo aux couleurs du fournisseur, valise à outils à roulettes, sacoche pour ordinateur portable, badge avec logo du fournisseur d'imprimantes",
            "props": [
                "Polo aux couleurs du fournisseur (Canon, Ricoh, Xerox ou HP)",
                "Valise à outils à roulettes avec outils de base et cartouches de toner",
                "Ordinateur portable pour les « diagnostics et mises à jour du firmware »",
                "Bon d'intervention imprimé faisant référence au modèle d'imprimante et à l'étage concerné",
                "Badge avec logo et photo de l'entreprise fournisseur",
            ],
            "script": "Bonjour, je suis {name} de chez {printer_vendor}. Nous avons reçu une alerte automatique\nindiquant que la {printer_model} à l'étage {floor} nécessite une maintenance et une mise à jour\ndu firmware. J'aurai besoin d'accéder directement à l'imprimante et de connecter mon\nordinateur portable pour la mise à jour du firmware. Je dois également vérifier la connectivité\nréseau côté serveur d'impression. Cela devrait prendre environ 45 minutes. Il est possible\nque je doive intervenir sur les imprimantes d'autres étages également — notre système en a\nsignalé plusieurs pour la même mise à jour.",
            "target_areas": ["Étages de bureaux avec imprimantes en réseau", "Salles d'impression et centres de reprographie", "Locaux réseau où les serveurs d'impression sont connectés", "Étages de la direction (cibles à forte valeur)", "Plusieurs étages pour une « maintenance groupée »"],
            "objectives": [
                "Installer un implant réseau via la connexion Ethernet de l'imprimante",
                "Accéder à plusieurs étages sous couvert d'une « maintenance groupée »",
                "Collecter des identifiants depuis le serveur d'impression ou les travaux d'impression en cache",
                "Installer un dispositif pirate sur les ports réseau à proximité des imprimantes",
                "Cartographier l'agencement des bureaux et identifier les postes de travail à forte valeur lors du parcours des étages",
            ],
        },
        {
            "id": "it_asset_inventory",
            "name": "Spécialiste de l'inventaire des actifs informatiques",
            "appearance": "Tenue professionnelle, bloc-notes avec feuille de suivi des actifs, lecteur de codes-barres, ordinateur portable, badge du service informatique sur un tour de cou",
            "props": [
                "Bloc-notes avec feuille d'inventaire des actifs imprimée montrant les enregistrements existants",
                "Lecteur de codes-barres USB pour « scanner les étiquettes d'actifs »",
                "Ordinateur portable avec un « logiciel de gestion des actifs » ouvert",
                "Appareil photo pour « photographier les étiquettes d'actifs et les numéros de série »",
                "Badge du service informatique (générique ou usurpé)",
            ],
            "script": "Bonjour, je suis {name} de la gestion des actifs informatiques. Nous réalisons l'inventaire\nmatériel annuel pour {company} — chaque poste de travail, écran et équipement réseau\ndoit être vérifié. J'ai besoin de scanner physiquement l'étiquette d'actif de chaque machine\net de vérifier que le numéro de série correspond à nos registres. Je passerai bureau par bureau\nà l'étage {floor} aujourd'hui. La gestion du bâtiment et votre responsable de service\nont été prévenus la semaine dernière. J'essaierai d'être rapide et de ne déranger personne.",
            "target_areas": ["Chaque bureau et poste de travail de l'étage ciblé", "Salles serveur et locaux réseau", "Salles de réunion avec équipement audiovisuel", "Locaux de stockage avec matériel de rechange", "Bureaux de la direction"],
            "objectives": [
                "Obtenir un accès prolongé et systématique à chaque poste de travail du bâtiment",
                "Photographier les écrans, les post-it et les tableaux blancs pour collecter des identifiants",
                "Connecter des dispositifs USB (keyloggers, implants) en « scannant les étiquettes d'actifs »",
                "Cartographier l'ensemble de la topologie réseau en documentant les appareils connectés",
                "Identifier les postes de travail non surveillés et non verrouillés pour exploitation",
            ],
        },
    ],
